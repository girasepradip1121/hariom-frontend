...
const Contact = () => {
  ...
  const fetchContacts = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${USER_BASE_URL}/api/contacts/getContact`, {
        headers: {
  ...
  const renderMessage = (message, id) => {
    ...
    return (
      <div className="flex flex-col">
        <div className={`text-sm text-gray-500 ${isExpanded ? "" : "line-clamp-2"}`}>
          {displayMessage}
        </div>
        {message.length > 100 && (
          <button
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <h1 className="text-2xl font-bold text-gray-800">Manage Contact Requests</h1>

...
const CombosPage = () => {
  ...
  const getProductDataFunction = async (page = currentPage, filters = {}) => {
    try {
      setLoading(true);
      const response = await axios.post(`${USER_BASE_URL}/api/products/all-products`, {
        page,
        perPage,
        filter: {
      ...
      const comboOnly = (response.data.products || []).filter((p) => p.combos === true);
  ...
  const handleAddToWishlist = async (productId, e) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || "Error adding product to wishlist");
    }
  };
  ...
  if (loading) {
    return (
      <section className="container mx-auto py-12 px-2 md:px-4 lg:px-10 xl:px-8">
        <h2 className="text-2xl md:text-3xl lg:text-4xl font-semibold mb-2">Combos</h2>
    ...
    <section className="container mx-auto py-12 px-2 md:px-4 lg:px-10 xl:px-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 xl:grid-cols-8 space-y-4 md:space-y-0 md:gap-4">
        ...
        <aside className="lg:col-span-2 xl:col-span-2 p-1 md:p-0 lg:p-4 py-4 border border-[#B7B4FF] rounded-lg space-y-4 text-sm bg-white h-fit">
          <div>
            <h3 className="font-semibold text-xl lg:text-2xl pt-4 px-4">Categories</h3>
          ...
          <div>
            <h3 className="font-semibold text-xl lg:text-2xl pt-4 px-4">SubCategories</h3>
            <ul className="p-2 px-4">
              {subCategoryList
          ...
          <div className="pb-2 px-4">
            <h3 className="font-semibold mb-2 text-xl lg:text-2xl">Price Range</h3>
              ...
              {[5, 4, 3].map((stars) => (
                <label key={stars} className="flex items-center space-x-2">
                  ...
                  <span className="text-2xl lg:text-3xl flex items-center">
                    ...
                    {stars !== 5 && <span className="text-base text-gray-400 ml-1"> & Up</span>}
                  </span>
                </label>
              ))}
            </div>
                ...
                >
                  ...
                  <div className="py-4 px-3">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <p className="text-[#393185] text-sm font-semibold">₹{product.price}</p>
                      {product.actualPrice > 0 && product.actualPrice > product.price && (
                        <p className="text-gray-500 text-xs line-through">₹{product.actualPrice}</p>
                      )}
                    </div>
                  </div>
                </div>

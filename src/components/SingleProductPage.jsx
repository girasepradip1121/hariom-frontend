...
const SingleProductPage = () => {
      ...
      try {
        ...
        if (typeof features === "string") {
          try {
            ...
            features = features
              ? features.split(",").map((item) => item.trim().replace(/^'|'$/g, ""))
              : [];
          }
        }
        ...
        if (typeof howToUse === "string") {
          try {
            ...
            howToUse = howToUse
              ? howToUse.split(",").map((item) => item.trim().replace(/^'|'$/g, ""))
              : [];
          }
        }
        howToUse = Array.isArray(howToUse) ? howToUse : howToUse ? [howToUse] : [];
  ...
  useEffect(() => {
    if (activeTab === "review") {
      const fetchReviews = async () => {
        try {
          const response = await axios.get(`${USER_BASE_URL}/api/ratings/products/${id}`);
  ...
  const handleQuantityChange = (increment) => {
    setQuantity((prev) => {
      ...
      if (product && newQuantity > product.originalQty) return product.originalQty;
      return newQuantity;
    });
  };
  ...
  return (
    <div className="py-12 px-2 md:px-4 lg:px-10 xl:px-8 container mx-auto">
      <div className="text-sm text-gray-500 mb-4">
        Home / Product / <span className="text-gray-900 font-medium">{product.name}</span>
      </div>
        ...
        <div className="flex flex-col gap-4 w-full lg:w-2/5">
          ...
          <div className="grid grid-cols-5 gap-2">
            {Array.isArray(product.images) && product.images.length > 0 ? (
              ...
              <div className="text-gray-500">No additional images available.</div>
            )}
          </div>
        </div>
        ...
        <div className="w-full lg:w-3/5 space-y-4">
          ...
          <div className="flex items-center gap-1 text-[#393185]">
            {[...Array(5)].map((_, i) => (
              <Star
                key={i}
                fill={i < Math.round(product.averageRatings) ? "currentColor" : "none"}
                className={i < Math.round(product.averageRatings) ? "" : "text-gray-800"}
              />
            ))}
          ...
          <p className="text-lg md:text-xl text-gray-600">{product.shortDescription}</p>

          <ul className="text-gray-600 list-disc list-inside space-y-1">
            <li>Free shipping on orders over ₹1500</li>
            <li className={product.remainingQty === 0 ? "text-red-600" : "text-gray-600"}>
          ...
          <div className="w-full flex gap-2">
            <button
              ...
              className={`w-1/2 bg-[#393185] text-white px-4 py-2 md:py-3 rounded-md md:text-lg text-center ${
                product.remainingQty === 0 ? "opacity-50 cursor-not-allowed" : "cursor-pointer"
              }`}
          ...
          {activeTab === "specification" && (
            <div className="w-full">
              <div className="grid grid-cols-2 text-sm md:text-lg text-gray-500 border-gray-200">
                <div className="py-3 px-4 border-b border-gray-200 mr-4">Volume</div>
                <div className="py-3 px-4 border-b border-gray-200">{product.volume || "N/A"}</div>
                <div className="py-3 px-4 border-b border-gray-200 mr-4">Ingredients</div>
                <div className="py-3 px-4 border-b border-gray-200">
                  {product.ingredients || "N/A"}
                </div>
                <div className="py-3 px-4 border-b border-gray-200 mr-4">Scent</div>
                <div className="py-3 px-4 border-b border-gray-200">{product.scent || "N/A"}</div>
                <div className="py-3 px-4 border-b border-gray-200 mr-4">pH Level</div>
                <div className="py-3 px-4 border-b border-gray-200">{product.phLevel || "N/A"}</div>
                <div className="py-3 px-4 border-b border-gray-200 mr-4">Shelf Life</div>
                <div className="py-3 px-4 border-b border-gray-200">
                  {product.shelfLife || "N/A"} months
                </div>
                <div className="py-3 px-4 border-b border-gray-200 mr-4">Made In</div>
                <div className="py-3 px-4 border-b border-gray-200">{product.madeIn || "N/A"}</div>
                <div className="py-3 px-4 border-b border-gray-200 mr-4">Packaging</div>
                <div className="py-3 px-4 border-b border-gray-200">
              ...
              {Array.isArray(reviews) && reviews.length > 0 ? (
                reviews.map((review, i) => (
                  <div key={i}>
                    ...
                    <div className="flex items-center space-x-1 text-[#393185] md:text-2xl">
                      {[...Array(5)].map((_, idx) => (
                        <span
                          key={idx}
                          className={
                            idx < Math.round(review.rating) ? "text-[#393185]" : "text-gray-300"
                          }
                        >
                          ★
                        </span>
                    ...
                    <p className="md:text-xl text-gray-700 mt-1">{review.description}</p>
